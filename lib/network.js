...
class HiddenSocket extends Duplex {
   ...
   * @param {module:granax/context~TorContext} context
   */
  constructor(context) {
    super();
  ...
  /**
   * @typedef {object} module:granax/network~HiddenSocket~ConnectOptions
   ...
   * tor SOCKSv5 proxy.
   * @param {module:granax/network~HiddenSocket~ConnectOptions} options - Destination connection address
   */ 
  connect(options) {
...
class HiddenServer {
   ...
   * @callback module:granax/network~HiddenServer~ConnectionListener
   * @param {net.Socket} connection
   */

  /**
   * Wraps a net.Server, exposing it as an onion service. 
   * @param {module:granax/context~TorContext} context - Tor context to create onion.
   * @param {module:granax/network~HiddenServer~ConnectionListener} [connectionLister] - Function to handle incoming connections 
   */
   ...
   * @param {module:granax/commands~ControlCommand~AddOnionOptions}
   * @returns {Promise<module:granax/network~HiddenServer~Address>}
   */
  ...
  /**
   * @typedef {object} module:granax/network~HiddenServer~Address
   ...
   * @returns {module:granax/network~HiddenServer~Address}
   */
  address() {
...
class TorHttpAgent extends Agent {
   ...
   * @param {module:granax/context~TorContext} context - Tor context to proxy through.
   */
  constructor(context) {
    super(options);
    this._context = context;
  ...
  /**
   * Implements http.Agent#createConnection to use a {@link module:granax/network~HiddenSocket}. 
   * You probably won't use this directly, instead passing this instance to http#request.
   * @param {module:granax/network~HiddenSocket~ConnectOptions} options
